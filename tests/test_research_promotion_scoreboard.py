...
def test_scoreboard_preserves_new_shadow_native_status_and_gate_progress(tmp_path: Path) -> None:
    ...
    under = board.loc["Projection Underperformer Shadow"]
    ...
    assert "beat_projection_rate" in crusher["Signal"]
    assert crusher["Recommended Action"] == "COLLECT_EXACT_PROJECTION_CRUSHER_EVIDENCE"
    assert crusher["Reason"] == "native Crusher gate reason"
    assert crusher["Production Authority"] == "NONE"

    assert under["Status"] == "LEARNING"
    assert "starts 44/60" in under["Gate Progress"]
    assert "below_projection_rate" in under["Signal"]
    assert under["Recommended Action"] == "COLLECT_EXACT_PROJECTION_UNDERPERFORMER_EVIDENCE"
    assert under["Reason"] == "native underperformer gate reason"
    assert under["Production Authority"] == "NONE"
