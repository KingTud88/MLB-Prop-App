...
def _center(secondary: str = "sample=1", lane: str = "Synthetic Lane") -> pd.DataFrame:
    return pd.DataFrame([{
        "Lane": lane,
        "Category": "TEST",
        ...
        "Command_Center_Version": "synthetic-command-center-v1",
    }])
...
def _write_pipeline(root: Path, base: pd.DataFrame, promotion: pd.DataFrame, history: pd.DataFrame, refresh: str) -> None:
    ...
    packet = build_manual_review_packet(digest, history, promotion, refresh)
...
def _patch_centers(monkeypatch, base: pd.DataFrame, promotion: pd.DataFrame) -> None:
    monkeypatch.setattr(freshness, "build_command_center", lambda _root: base.copy())
    monkeypatch.setattr(freshness, "build_promotion_command_center", lambda _root: promotion.copy())
...
def test_exact_recomputation_marks_full_all_lane_pipeline_current(tmp_path: Path, monkeypatch) -> None:
    base = _center(lane="Base Lane")
    promotion = pd.concat([base, _center(lane="Extra Promotion Lane")], ignore_index=True)
    history = append_history(promotion, observed_at_utc="2026-08-18T12:00:00+00:00")
    refresh = "2026-08-18T13:00:00+00:00"
    _write_pipeline(tmp_path, base, promotion, history, refresh)
    _patch_centers(monkeypatch, base, promotion)
...
def test_promotion_center_detects_extra_lane_drift_even_when_base_center_is_current(tmp_path: Path, monkeypatch) -> None:
    base = _center(lane="Base Lane")
    saved_promotion = base.copy()
    expected_promotion = pd.concat([base, _center(lane="New Shadow Lane")], ignore_index=True)
    history = append_history(expected_promotion, observed_at_utc="2026-08-18T12:00:00+00:00")
    refresh = "2026-08-18T13:00:00+00:00"
    _write_pipeline(tmp_path, base, saved_promotion, history, refresh)
    _patch_centers(monkeypatch, base, expected_promotion)
    ...
    assert audit.loc["PROMOTION_COMMAND_CENTER", "Freshness_Status"] == freshness.SOURCE_NEWER
    assert "New Shadow Lane" in audit.loc["PROMOTION_COMMAND_CENTER", "Detail"]
    assert audit.loc["HISTORY", "Freshness_Status"] == freshness.UPSTREAM_STALE
...
def test_multicell_injection_and_later_human_close_remain_fresh(tmp_path: Path, monkeypatch) -> None:
    ...
    injected = inject_multicell_reviews(base_packet, pd.DataFrame(columns=QUEUE_COLUMNS), calibration, pd.DataFrame(), refresh)
    ...
    build_packet_summary(injected, digest, refresh).to_csv(tmp_path / "research_manual_review_packet_summary.csv", index=False)
    ...
    build_queue_summary(queue, reviewed_at).to_csv(tmp_path / "research_manual_review_queue_summary.csv", index=False)
...
def test_base_command_center_detects_source_mismatch_without_time_thresholds(tmp_path: Path, monkeypatch) -> None:
    ...
    refresh = "2026-08-18T13:00:00+00:00"
    _write_pipeline(tmp_path, saved, expected, history, refresh)
    _patch_centers(monkeypatch, expected, expected)
...
def test_history_mismatch_propagates_upstream_stale_to_digest_packet_and_queue(tmp_path: Path, monkeypatch) -> None:
    ...
    refresh = "2026-08-18T13:00:00+00:00"
    _write_pipeline(tmp_path, current, current, history, refresh)
    _patch_centers(monkeypatch, current, current)
...
def test_digest_detects_newer_history_transition_than_exact_refresh(tmp_path: Path, monkeypatch) -> None:
    old = _center("sample=1")
    current = _center("sample=2")
    history = append_history(old, observed_at_utc="2026-08-18T12:00:00+00:00")
...
def test_freshness_contract_is_report_only_all_lane_and_has_no_card_lock() -> None:
