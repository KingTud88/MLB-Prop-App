...
.sok-sidebar-logo{display:flex!important;justify-content:center!important;align-items:center!important;height:100px!important;margin:0!important}.sok-sidebar-logo img{width:150px!important;height:100px!important;object-fit:contain!important;display:block!important}
.sok-sidebar-title{font-size:1.3rem!important;line-height:1!important}.sok-sidebar-sub{font-size:.72rem!important;line-height:1.25!important;margin:.25rem 0 .4rem!important}
.sok-nav{gap:.08rem!important;margin:.2rem 0 .45rem!important}.sok-nav a,.sok-disabled-nav{padding:.36rem .45rem!important;font-size:.78rem!important;border-radius:8px!important;color:#dce6f0!important;font-weight:800!important}.sok-disabled-nav{display:block!important}
.sok-search-title{margin-top:.45rem!important;font-size:.85rem!important}.sok-lock{font-size:.68rem!important;line-height:1.25!important;margin-top:.25rem!important}.sok-date{margin-top:.4rem!important;padding:.55rem!important}.sok-side-card{margin-top:.45rem!important;padding:.6rem!important}
...
def _sok_math_path(log,game,manual):
    ...
    k_rate=shrink(raw_k_rate,total_bf,.224,120.0)
    ...
    projected_outs=float(np.clip(outs*pitch_limit_factor*rest_factor,3.0,24.0))
    projected_k=projected_bf*k_rate*opponent_factor*park_factor*ump_factor*weather_factor
    projected_k=float(np.clip(.78*projected_k+.22*weighted_mean(starts["strikeouts"],5,5.0),.5,13.5))
...
def _sok_simulation_path(log,game,manual,simulations,seed):
    ...
    weights=np.exp(-.08*ages); weights=weights/weights.sum()
    rng=np.random.default_rng(seed)
    idx=rng.choice(n,size=simulations,p=weights)
    bf_hist=starts["batters_faced"].to_numpy(float)
    outs_hist=starts["outs"].to_numpy(float)
    bf_base=bf_hist[idx]; outs_base=outs_hist[idx]
    bf_sd=float(starts["batters_faced"].std(ddof=1)) if n>2 else 3.5
    outs_sd=float(starts["outs"].std(ddof=1)) if n>2 else 3.0
    ...
    latent_k_rate=rng.beta(alpha,beta,size=simulations)
    opponent_factor=manual["opponent_k_pct"]/22.4
    park_factor=PARK_K_FACTOR.get(game.venue,1.0)
    ump_factor=manual["umpire_k_factor"]; weather_factor=manual["weather_factor"]; rest_factor=manual["rest_factor"]
    mean_pitch=float(weighted_mean(starts["pitches"],5.0,88.0))
    pitch_limit_factor=float(np.clip(manual["pitch_limit"]/max(mean_pitch,75.0),.78,1.12))
    adjusted_rate=np.clip(latent_k_rate*opponent_factor*park_factor*ump_factor*weather_factor,.02,.55)
    sim_bf=np.clip(np.rint(bf_base+rng.normal(0.0,max(bf_sd*.35,1.0),size=simulations)),12,35).astype(int)
    sim_bf=np.clip(np.rint(sim_bf*pitch_limit_factor*rest_factor),8,35).astype(int)
    sim_k=rng.binomial(sim_bf,adjusted_rate).astype(float)
    sim_outs=np.clip(np.rint(outs_base+rng.normal(0.0,max(outs_sd*.35,1.0),size=simulations)),3,27).astype(float)
    sim_outs=np.clip(np.rint(sim_outs*pitch_limit_factor*rest_factor),3,27)
...
def _sok_hist_probs(samples,maximum):
    values=np.clip(np.rint(samples).astype(int),0,maximum)
    counts=np.bincount(values,minlength=maximum+1).astype(float)
    return counts/counts.sum()

def calculate_projection(log,game,manual,simulations)->Projection:
    simulations=max(int(simulations),25000)
    math_path=_sok_math_path(log,game,manual)
    seed=int(hashlib.sha256(f"{game.key}|{date.today()}|{APP_VERSION}|two-path-v3".encode()).hexdigest()[:8],16)
    sim_path=_sok_simulation_path(log,game,manual,simulations,seed)
    rng=np.random.default_rng(seed+1)
    math_k=rng.choice(np.arange(len(math_path["k_probs"])),size=simulations,p=math_path["k_probs"])
    math_o=rng.choice(np.arange(len(math_path["outs_probs"])),size=simulations,p=math_path["outs_probs"])
    take_sim=rng.random(simulations)<.5
    k_samples=np.where(take_sim,sim_path["k_samples"],math_k)
    outs_samples=np.where(take_sim,sim_path["outs_samples"],math_o)
    k_probs=_sok_hist_probs(k_samples,18); outs_probs=_sok_hist_probs(outs_samples,27)
    quality=min(100,35+len(math_path["starts"])*2+(15 if math_path["total_bf"]>=250 else 0)+(10 if game.pitcher_id else 0))
    confidence="High" if quality>=85 else "Medium" if quality>=65 else "Low"
    factors=[("Opponent strikeout profile",math_path["opponent_factor"]-1),("Recent workload / pitch limit",math_path["pitch_limit_factor"]-1),("Park",math_path["park_factor"]-1),("Umpire",math_path["ump_factor"]-1),("Weather",math_path["weather_factor"]-1),("Rest",math_path["rest_factor"]-1)]
...
TWO_PATH_PANEL = """
st.markdown(
f'<div class=\"sok-two-path\"><div class=\"sok-two-path-title\">TWO-PATH PROJECTION ENGINE</div>'
f'<div class=\"sok-two-path-grid\">'
f'<div class=\"sok-two-path-cell\"><div class=\"sok-two-path-kicker\">PATH 1 - 25,000 SIMULATED GAMES</div>'
f'<div class=\"sok-two-path-value\">K {projection.sim_mean_k:.2f} · OUTS {projection.sim_mean_outs:.2f}</div>'
f'<div class=\"sok-two-path-detail\">Recency-weighted historical workloads, empirical-Bayes pitcher skill, opponent/park/context adjustments, and game-level binomial strikeout draws.</div></div>'
f'<div class=\"sok-two-path-cell\"><div class=\"sok-two-path-kicker\">PATH 2 - MATHEMATICAL MODEL</div>'
f'<div class=\"sok-two-path-value\">K {projection.math_mean_k:.2f} · OUTS {projection.math_mean_outs:.2f}</div>'
f'<div class=\"sok-two-path-detail\">Weighted recent form, empirical-Bayes shrinkage, context factors, Negative Binomial strikeout distribution, and bounded outs distribution.</div></div>'
f'<div class=\"sok-two-path-cell\"><div class=\"sok-two-path-kicker\">FINAL ENSEMBLE - 50/50</div>'
f'<div class=\"sok-two-path-value sok-two-path-ensemble\">K {projection.mean_k:.2f} · OUTS {projection.mean_outs:.2f}</div>'
f'<div class=\"sok-two-path-detail\">Final probabilities, ranges, fair odds, and ladder are generated from the blended path distributions.</div></div></div></div>',
unsafe_allow_html=True,
)
"""
source=re.sub(r'(^\s*projection=calculate_projection\([^\n]+\)\n)',lambda m:m.group(1)+TWO_PATH_PANEL,source,count=1,flags=re.M)
