...
def fetch_dynamic_opposing_lineup(team_abbr):
    ...
    if live_names:
        ...
        for i, name in enumerate(live_names): lineup_rows.append({"Name": name, "K%": dynamic_ks[i]})
    else:
        ...
        for i, name in enumerate(fake_names): lineup_rows.append({"Name": f"{team_abbr} {name}" if "Hitter" in name else name, "K%": dynamic_ks[i]})
    ...
    display_df = pd.DataFrame({
        ...
        "K% USED": lineup_df["K%"], "VS HAND": [round(k * 0.95, 1) for k in lineup_df["K%"]], "SEASON": lineup_df["K%"]
    })
