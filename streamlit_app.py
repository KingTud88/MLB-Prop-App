...
.sok-sidebar-logo>*{display:none!important}.sok-sidebar-title,.sok-sidebar-sub{display:none!important}
...
.section-frame{border:0!important;border-radius:0!important;padding:0!important;margin-top:1.15rem!important;background:transparent!important;box-shadow:none!important}.section-ribbon{position:relative!important;z-index:5!important;margin:0 auto .72rem!important}
...
@media(max-width:900px){.sok-hero{display:block!important;min-height:0!important}.sok-hero .sok-hero-mascot{width:180px!important;height:155px!important;margin:0 auto}.sok-hero .sok-hero-mascot img{width:155px!important;height:155px!important}.sok-hero-title{text-align:center!important}.sok-title{font-size:3.6rem!important}.sok-hero-right{justify-content:center!important;margin-top:16px!important}.two-path-grid{grid-template-columns:1fr 1fr}.two-path-head:nth-child(1),.two-path-cell:nth-child(4n+1){grid-column:1/-1}}
</style>
"""
source = source.replace('st.markdown(r"""', 'st.markdown(BRANDING_FIX_CSS, unsafe_allow_html=True)\n\nst.markdown(r"""', 1)

# Locked navigation: Odds API remains merged into Projection and is not a sidebar item.
...
# Deterministic hero matching the approved reference.
...
st.markdown(f'''<div class="sok-hero">
  ...
  <div class="sok-hero-right"><div class="sok-built-badge"><div>BUILT FOR</div><div class="cle">CLE</div><div>BASEBALL</div><div class="stars">★★★</div></div><div class="sok-status"><div class="head">DATA STATUS</div><div class="live">● {projection.confidence.upper()}</div><div class="quality">High confidence<br>Data quality {pct}/100</div><div class="bar"><span style="width:{pct}%"></span></div></div></div>
...
# Two-path engine: mathematical model + >=25,000 simulated games + 50/50 ensemble.
...
def _sok_simulated_path(log, game, manual, simulations, seed):
    ...
    n = len(starts)
    ages = np.arange(n - 1, -1, -1, dtype=float); weights = np.exp(-0.08 * ages); weights /= weights.sum()
    ...
    alpha = max(0.224 * 120.0 + total_k, 0.5); beta = max(0.776 * 120.0 + total_bf - total_k, 0.5)
    ...
    mean_pitch = weighted_mean(starts["pitches"], 5.0, 88.0); limit = float(np.clip(manual["pitch_limit"] / max(mean_pitch, 75.0), 0.78, 1.12))
    ...
    k = rng.binomial(sampled_bf, rate).astype(float); o = np.clip(np.rint(outs + rng.normal(0, max(out_sd * 0.35, 1.0), simulations)), 3, 27); o = np.clip(np.rint(o * limit * rest), 3, 27).astype(float)
    return k, o
...
def calculate_projection(log, game, manual, simulations):
    ...
    seed = int(hashlib.sha256(f"{game.key}|{date.today()}|{APP_VERSION}|two-path-v8".encode()).hexdigest()[:8], 16)
    ...
    final_k = np.where(use_sim, sim_k, math_k).astype(float); final_outs = np.where(use_sim, sim_outs, math_outs).astype(float)
    ...
    TWO_PATH_DETAILS[game.key] = {"sim_k":float(sim_k.mean()),"sim_outs":float(sim_outs.mean()),"sim_k_sd":float(sim_k.std(ddof=1)),"sim_outs_sd":float(sim_outs.std(ddof=1)),"math_k":float(math_projection.mean_k),"math_outs":float(math_projection.mean_outs),"math_k_sd":float(math_projection.k_sd),"math_outs_sd":float(math_projection.outs_sd),"ensemble_k":float(final_k.mean()),"ensemble_outs":float(final_outs.mean()),"simulations":simulations}
...
# Replace the old Probability Table only; keep Projection Drivers on the right.
...
new_probability = '''with left:
    ...
    if tp:
        sim_k,sim_o=tp["sim_k"],tp["sim_outs"];math_k,math_o=tp["math_k"],tp["math_outs"];ens_k,ens_o=tp["ensemble_k"],tp["ensemble_outs"];n_sims=tp["simulations"]
        k_delta=sim_k-math_k;o_delta=sim_o-math_o
        st.markdown(f'''<div class="two-path-panel"><div class="two-path-title">TWO-PATH PROJECTION ANALYSIS</div><div class="two-path-grid"><div class="two-path-head">PATH</div><div class="two-path-head">STRIKEOUTS</div><div class="two-path-head">OUTS</div><div class="two-path-head">WEIGHT</div><div class="two-path-cell"><span class="two-path-kicker">Mathematical Model</span></div><div class="two-path-cell"><strong>{math_k:.2f}</strong></div><div class="two-path-cell"><strong>{math_o:.2f}</strong></div><div class="two-path-cell">50%</div><div class="two-path-cell"><span class="two-path-kicker">Simulated Games</span><div class="two-path-muted">{n_sims:,} games</div></div><div class="two-path-cell"><strong>{sim_k:.2f}</strong></div><div class="two-path-cell"><strong>{sim_o:.2f}</strong></div><div class="two-path-cell">50%</div><div class="two-path-cell"><span class="two-path-kicker">Final Ensemble</span></div><div class="two-path-cell"><strong class="two-path-good">{ens_k:.2f}</strong></div><div class="two-path-cell"><strong class="two-path-good">{ens_o:.2f}</strong></div><div class="two-path-cell">50/50</div></div><div class="two-path-foot">Path agreement: K <span class="two-path-delta">{k_delta:+.2f}</span> • Outs <span class="two-path-delta">{o_delta:+.2f}</span> &nbsp;|&nbsp; Final projection uses the locked 50/50 ensemble.</div></div>''',unsafe_allow_html=True)
    else:
...
if old_probability not in source:
    st.error("Target Probability Table block was not found; refusing to render a partial redesign.")
    st.stop()
