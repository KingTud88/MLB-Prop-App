...
with st.sidebar:
    ...
    pitcher_input = st.text_input("Enter Pitcher Name", "Eury Perez")
...
def fetch_pitcher_intel_metrics(pitcher_name):
    try:
        ...
        if pitcher_data.empty:
            all_pitchers = pitching_stats_bref(2024)
            all_pitchers['Name_Lower'] = all_pitchers['Name'].str.lower()
            pitcher_data = all_pitchers[all_pitchers['Name_Lower'].str.contains(pitcher_name.lower(), na=False)]
...
def fetch_dynamic_opposing_lineup(team_abbr):
    try:
        ...
        if team_hitters.empty: team_hitters = all_hitters.head(15).copy()
        ...
        if live_names:
            ...
            for name in live_names:
                ...
                if not match_row.empty: lineup_data = pd.concat([lineup_data, match_row.head(1)])
                else: lineup_data = pd.concat([lineup_data, pd.DataFrame([{"Name": name, "AB": 100, "SO": 22, "PA": 100}])])
        else:
            status_msg = f"⏳ Lineup Status: Orders pending. Season depth charts active for {team_abbr}."
        ...
        return fallback_df, f"⚠️ Fallback active: {str(e)}"
...
with col1:
    ...
    st.caption(f"⚾ Matchup Intel Summary | RHP vs {opposing_team}")
    ...
    if p_stats is not None and not isinstance(p_stats, pd.DataFrame):
        ...
        with c_p1: st.markdown(f"<div class='metric-card'><div class='metric-label'>PROJ K</div><div class='metric-value'>{live_avg}</div><div style='color:#50FA7B;font-size:12px;'>+{diff_val} vs {sportsbook_line}</div></div>", unsafe_allow_html=True)
        with c_p2:
            rec_tag = "OVER" if live_avg > sportsbook_line else "UNDER"
            st.markdown(f"<div class='metric-card'><div class='metric-label'>RECOMMENDATION</div><div class='metric-value' style='color:#50FA7B;'>{rec_tag}</div><div style='font-size:12px;color:#8BE9FD;'>{sportsbook_line} Ks Line</div></div>", unsafe_allow_html=True)
            
        st.subheader("📊 Advanced Profile Analytics")
        c_m1, c_m2, c_m3, c_m4 = st.columns(4)
        with c_m1: st.markdown("<div class='metric-card'><div class='metric-label'>K GRADE</div><div class='tag-grade'>B</div></div>", unsafe_allow_html=True)
        with c_m2: st.markdown(f"<div class='metric-card'><div class='metric-label'>CEILING</div><div class='metric-value' style='color:#50FA7B;'>{int(live_avg + 2)}K</div></div>", unsafe_allow_html=True)
        with c_m3: st.markdown("<div class='metric-card'><div class='metric-label'>TOP PITCH</div><div style='font-size:11px;font-weight:bold;color:#BD93F9;margin-top:5px;'>Sinker<br>45% use</div></div>", unsafe_allow_html=True)
        with c_m4: st.markdown(f"<div class='metric-card'><div class='metric-label'>ARSENAL</div><div class='metric-value'>{int(p_stats['SO'])}</div></div>", unsafe_allow_html=True)
        ...
        st.markdown(f"<div class='metric-card' style='border:1px solid #50FA7B;color:#50FA7B;font-weight:bold;'>💡 Historical database logs successfully unlocked for {pitcher_input.title()}.</div>", unsafe_allow_html=True)
...
with col2:
    ...
    if lineup_df is not None and not lineup_df.empty:
        st.dataframe(lineup_df.style.background_gradient(subset=["K% USED", "VS HAND", "SEASON"], cmap="Purples"), use_container_width=True, hide_index=True)
        st.info(app_status)
