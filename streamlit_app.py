...
# Keep the approved sidebar order and Projection-first navigation.
...
def _sok_math_path(log,game,manual):
    ...
    total_bf=float(starts["batters_faced"].sum()); raw_k_rate=float(starts["strikeouts"].sum()/max(total_bf,1)); k_rate=shrink(raw_k_rate,total_bf,.224,120.0)
    opponent_factor=manual["opponent_k_pct"]/22.4; park_factor=PARK_K_FACTOR.get(game.venue,1.0); ump_factor=manual["umpire_k_factor"]; weather_factor=manual["weather_factor"]; rest_factor=manual["rest_factor"]
    pitch_limit_factor=float(np.clip(manual["pitch_limit"]/max(pitches,75.0),.78,1.12)); projected_bf=bf*pitch_limit_factor*rest_factor; projected_outs=outs*pitch_limit_factor*rest_factor
    projected_k=projected_bf*k_rate*opponent_factor*park_factor*ump_factor*weather_factor; projected_k=float(np.clip(.78*projected_k+.22*weighted_mean(starts["strikeouts"],5,5.0),.5,13.5)); projected_outs=float(np.clip(projected_outs,3.0,24.0))
    k_variance=float(starts["strikeouts"].var(ddof=1)) if len(starts)>2 else projected_k*1.25; dispersion=max((k_variance-projected_k)/max(projected_k**2,.1),.08); k_probs=negbin_pmf(projected_k,dispersion,18)
    outs_sd=float(starts["outs"].std(ddof=1)) if len(starts)>2 else 4.0; outs_sd=float(np.clip(outs_sd,2.5,6.5)); outs_probs=discrete_normal_probs(projected_outs,outs_sd,27)
...
def _sok_simulation_path(log,game,manual,simulations,seed):
    ...
    ages=np.arange(n-1,-1,-1,dtype=float); weights=np.exp(-.08*ages); weights=weights/weights.sum(); rng=np.random.default_rng(seed); idx=rng.choice(n,size=simulations,p=weights)
    bf_hist=starts["batters_faced"].to_numpy(float); outs_hist=starts["outs"].to_numpy(float); bf_base=bf_hist[idx]; outs_base=outs_hist[idx]
    bf_sd=float(starts["batters_faced"].std(ddof=1)) if n>2 else 3.5; outs_sd=float(starts["outs"].std(ddof=1)) if n>2 else 3.0; total_bf=float(starts["batters_faced"].sum()); total_k=float(starts["strikeouts"].sum())
    alpha=max(.224*120.0+total_k,.5); beta=max(.776*120.0+total_bf-total_k,.5); latent_k_rate=rng.beta(alpha,beta,size=simulations)
    opponent_factor=manual["opponent_k_pct"]/22.4; park_factor=PARK_K_FACTOR.get(game.venue,1.0); ump_factor=manual["umpire_k_factor"]; weather_factor=manual["weather_factor"]; rest_factor=manual["rest_factor"]
    mean_pitch=float(weighted_mean(starts["pitches"],5.0,88.0)); pitch_limit_factor=float(np.clip(manual["pitch_limit"]/max(mean_pitch,75.0),.78,1.12)); adjusted_rate=np.clip(latent_k_rate*opponent_factor*park_factor*ump_factor*weather_factor,.02,.55)
    sim_bf=np.clip(np.rint(bf_base+rng.normal(0.0,max(bf_sd*.35,1.0),size=simulations)),12,35).astype(int); sim_bf=np.clip(np.rint(sim_bf*pitch_limit_factor*rest_factor),8,35).astype(int); sim_k=rng.binomial(sim_bf,adjusted_rate).astype(float)
    sim_outs=np.clip(np.rint(outs_base+rng.normal(0.0,max(outs_sd*.35,1.0),size=simulations)),3,27).astype(float); sim_outs=np.clip(np.rint(sim_outs*pitch_limit_factor*rest_factor),3,27)
    return {"k_samples":sim_k,"outs_samples":sim_outs,"mean_k":float(np.mean(sim_k)),"mean_outs":float(np.mean(sim_outs)),"k_sd":float(np.std(sim_k,ddof=1)),"outs_sd":float(np.std(sim_outs,ddof=1)),"pitch_limit_factor":pitch_limit_factor,"opponent_factor":opponent_factor,"park_factor":park_factor,"ump_factor":ump_factor,"weather_factor":weather_factor,"rest_factor":rest_factor}
...
def calculate_projection(log,game,manual,simulations)->Projection:
    simulations=max(int(simulations),1000); math_path=_sok_math_path(log,game,manual); seed_text=f"{game.key}|{date.today()}|{APP_VERSION}|two-path-v1"; seed=int(hashlib.sha256(seed_text.encode()).hexdigest()[:8],16); sim_path=_sok_simulation_path(log,game,manual,simulations,seed)
    rng=np.random.default_rng(seed+1); take_sim=rng.random(simulations)<.5; math_k=rng.choice(np.arange(len(math_path["k_probs"])),size=simulations,p=math_path["k_probs"]); math_o=rng.choice(np.arange(len(math_path["outs_probs"])),size=simulations,p=math_path["outs_probs"])
    k_samples=np.where(take_sim,sim_path["k_samples"],math_k); outs_samples=np.where(take_sim,sim_path["outs_samples"],math_o); k_probs=_sok_hist_probs(k_samples,18); outs_probs=_sok_hist_probs(outs_samples,27)
    quality=min(100,35+len(math_path["starts"])*2+(15 if math_path["total_bf"]>=250 else 0)+(10 if game.pitcher_id else 0)); confidence="High" if quality>=85 else "Medium" if quality>=65 else "Low"
