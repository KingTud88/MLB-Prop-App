...
SESSION.headers.update({"Accept": "application/json", "User-Agent": "StrikeOutKing9000/3.7.0"})
...
def _pitcher_workload_from_boxscore(data: dict, player_id: str) -> tuple[int | None, int | None]:
    for side in ("away", "home"):
        player = data.get("teams", {}).get(side, {}).get("players", {}).get(player_id)
        pitching = (player or {}).get("stats", {}).get("pitching", {})
        if pitching:
