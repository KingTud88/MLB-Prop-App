...
def calibrate_outs_blend(
    ...
    if frame.empty or not {sim_col, math_col, actual_col}.issubset(frame.columns):
        return OutsCalibrationResult(0.50, 0.50, 0, None, False, "Outs calibration fields are not populated yet.")
    ...
    if n < min_observations:
        return OutsCalibrationResult(0.50, 0.50, n, None, False, f"Need {min_observations} resolved outs observations; only {n} are available.")
    ...
    return OutsCalibrationResult(final_w, 1.0 - final_w, n, final_score, True, "Weight learned from resolved frozen total-outs projections.")
