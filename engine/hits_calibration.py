...
def hits_calibration_report(
    ...
    for line in lines:
        ...
        sim_col = f"hits_sim_over_{key}"
        math_col = f"hits_math_over_{key}"
        cols = {sim_col, math_col, "actual_hits_allowed"}
        if frame.empty or not cols.issubset(frame.columns):
            ...
            continue
        data = frame[[sim_col, math_col, "actual_hits_allowed"]].dropna()
        cal = calibrate_hits_blend(frame, line, min_observations=min_observations)
        rows.append({
            "Line": line,
