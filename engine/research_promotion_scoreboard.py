...
def render_research_promotion_scoreboard(root: Path) -> None:
    ...
    st.markdown(
        ...
        .research-review-closed{color:#9ee8ff;border:1px solid rgba(74,191,230,.62);background:rgba(10,65,83,.28);box-shadow:0 0 0 1px rgba(74,191,230,.06)}
    ...
    st.markdown(
        ...
        '<div class="research-board-copy">Every registered promotion lane is shown. Supporting diagnostics remain in their native reports. Native verdicts and readiness stay source-owned; this board never auto-promotes or activates a model feature. Once a human review is closed, the card header shows that completed governance state first while the immutable native source status remains visible underneath. Lane drilldowns follow this board; deeper calibration, workload, lineup, and signal diagnostics remain afterward.</div>'
        '</div>',
        unsafe_allow_html=True,
    )
    ...
    c.metric("Source-ready lanes", int(ready.sum()), help="Source-owned maturity/readiness. Human review closure is a separate governance record and becomes the primary card badge once completed.")
    ...
    for start in range(0, len(board), 2):
        ...
        for col, (_, row) in zip(cols, board.iloc[start:start + 2].iterrows()):
            ...
            primary_status_html = (
                '<div class="research-pill research-review-closed">HUMAN REVIEW CLOSED</div>'
                if review
                else f'<div class="research-pill {_status_class(status)}">{escape(status)}</div>'
            )
            review_html = ""
            source_status_html = ""
            if review:
                review_html = (
                    ...
                    '<span class="research-human-review-label">Human disposition</span>'
                    f'<span class="research-human-review-disposition">{escape(review["disposition"])}</span>'
                    '</div>'
                )
                source_status_html = (
                    '<div class="research-source-status">'
                    '<span class="research-source-status-label">Source status</span>'
                    f'<span class="research-source-status-value">{escape(status)}</span>'
                    '</div>'
                )
            with col:
                st.markdown(
                    ...
                    f'{review_html}{source_status_html}'
    ...
    st.caption("Projection History flow: lane drilldowns are next; deep diagnostics follow. Closed human review is the primary operational card state; immutable source status remains secondary context and is never rewritten.")
    with st.expander("ⓘ How to read the Research Promotion Command Center", expanded=False):
        ...
        st.markdown("**Human Review is a separate governance record.** On reviewed lanes, HUMAN REVIEW CLOSED becomes the primary card badge while the native source status remains explicitly displayed as secondary context.")
