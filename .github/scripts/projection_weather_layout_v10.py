...
marker = "PROJECTION_WEATHER_SUMMARY_V10"

if marker not in app:
    anchor = "/* PROJECTION_EMBLEM_MASTER_V9 · 128px master tiles rendered down for sharper edges */"
    start = app.find(anchor)
    if start < 0:
        raise SystemExit("Projection emblem V9 anchor not found; refusing to patch unrelated CSS")
    style_end = app.find("</style>", start)
    if style_end < 0:
        raise SystemExit("Projection style close not found")
    css = r'''
...
'''
    app = app[:style_end] + css + app[style_end:]

weather_line = 'weather_marker=f" {weather_risk.icon}" if weather_risk.icon else ""'
weather_vars = '''weather_marker=f" {weather_risk.icon}" if weather_risk.icon else ""
...
if '_weather_action={"HIGH":"AVOID · DELAY RISK"' not in app:
    if weather_line not in app:
        raise SystemExit("Weather marker anchor not found")
    app = app.replace(weather_line, weather_vars, 1)

old_alert = '''if weather_risk.available and weather_risk.level in {"HIGH","ELEVATED"}:
...
'''
if old_alert in app:
    app = app.replace(old_alert, "", 1)

old_hits = '''h1,h2=st.columns(2)
...
'''
new_hits = '''h1,h2,h3=st.columns([1,1,2])
with h1: st.markdown(f'<div class="metric-card"><div class="cc-card-top"><div class="cc-card-icon cc-emblem contact" aria-hidden="true"></div><div class="metric-label">PROJECTED HITS ALLOWED</div></div><div class="metric-value">{hits_proj.ensemble_mean:.2f}</div><span class="badge">↑ 80% RANGE {int(np.quantile(hits_proj.simulation_samples,.1))}-{int(np.quantile(hits_proj.simulation_samples,.9))}</span></div>',unsafe_allow_html=True)
render_reco(h2,hit_reco,key_prefix=f"manual_hits:{game.key}",market_key="pitcher_hits_allowed",proj=proj,hits_proj=hits_proj)
with h3:
    st.markdown(
...
'''
if "h1,h2,h3=st.columns([1,1,2])" not in app:
    if old_hits not in app:
        raise SystemExit("Hits Allowed two-column block not found; refusing to guess")
    app = app.replace(old_hits, new_hits, 1)

app_path.write_text(app, encoding="utf-8")

ui_path = Path("engine/ui_command_center.py")
ui = ui_path.read_text(encoding="utf-8")
if "cc-weather-status-icon" not in ui:
    css_anchor = '        .cc-matchup-status-meta { margin-top:.28rem;color:#afc0cf;font-family:var(--cc-ui-font);font-size:.84rem; }'
    ...
    if css_anchor not in ui:
        raise SystemExit("Matchup status CSS anchor not found")
    ui = ui.replace(css_anchor, css_add, 1)

old_weather = '    weather = f" {_safe(weather_icon)}" if weather_icon else ""'
new_weather = '    weather = f\'<span class="cc-weather-status-icon" aria-label="Weather delay risk">{_safe(weather_icon)}</span>\' if weather_icon else ""'
if 'aria-label="Weather delay risk"' not in ui:
    if old_weather not in ui:
        raise SystemExit("Matchup weather variable anchor not found")
    ui = ui.replace(old_weather, new_weather, 1)

old_name = '<div class="cc-matchup-name">{_safe(pitcher_name)}{weather}</div>'
new_name = '<div class="cc-matchup-name">{_safe(pitcher_name)}</div>'
if old_name in ui:
    ui = ui.replace(old_name, new_name, 1)

old_status = '<div class="cc-matchup-status-label">Game Status</div>'
new_status = '<div class="cc-matchup-status-label">Game Status{weather}</div>'
if new_status not in ui:
    if old_status not in ui:
        raise SystemExit("Game Status label anchor not found")
    ui = ui.replace(old_status, new_status, 1)

ui_path.write_text(ui, encoding="utf-8")
print("Applied compact Hits row, weather delay-risk card, and Game Status weather signal.")