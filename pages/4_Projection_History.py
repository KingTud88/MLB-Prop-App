...
st.caption("Frozen pregame StrikeOut King 9000 projections, resolved against final MLB strikeouts and hits allowed.")
...
col2.metric("Resolved games", int((k_resolved | h_resolved).sum()))
...
display["status"] = display.apply(
    lambda r: "Resolved" if pd.notna(r.get("actual_strikeouts")) or pd.notna(r.get("actual_hits_allowed")) else "Pending",
    axis=1,
)
