...
def _key(row: pd.Series | dict[str, object]) -> tuple[int, int, str, str] | None:
...
def merge_score_log(existing: pd.DataFrame | None, current: pd.DataFrame | None) -> pd.DataFrame:
    """Preserve the first frozen score for each exact pregame context and append only unseen contexts."""
    existing = existing.copy() if existing is not None else pd.DataFrame(columns=COLUMNS)
    current = current.copy() if current is not None else pd.DataFrame(columns=COLUMNS)
    for frame in (existing, current):
    ...
    for frame in (existing, current):
        for _, row in frame.iterrows():
            key = _key(row)
            if key is None:
                # Retain malformed historical records for auditability, but they cannot become keyed evidence.
                if frame is existing:
...
def main() -> None:
    ...
    result = merge_score_log(existing, current)
